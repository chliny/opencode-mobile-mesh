    ...
    const connect = async () => {
      ...
      current.onmessage = (event) => {
        if (typeof event.data === "string") {
          append(activeID, event.data)
          return
        }
        if (event.data instanceof ArrayBuffer) {
          const bytes = new Uint8Array(event.data)
          if (bytes[0] === 0) return
          append(activeID, new TextDecoder().decode(bytes))
        }
      }
    }
    void connect()
      ...
      {!client ? <Text style={[styles.empty, isDark && styles.textDark]}>{t("terminal.noConnection")}</Text> : !active ? <Text style={[styles.empty, isDark && styles.textDark]}>{t("terminal.empty")}</Text> : (
        <>
          ...
          <View style={[styles.inputBar, isDark && styles.borderDark]}>
            <TextInput value={input} onChangeText={setInput} onSubmitEditing={send} placeholder={t("terminal.inputPlaceholder")} placeholderTextColor="#777" style={[styles.input, isDark && styles.textDark]} autoCapitalize="none" autoCorrect={false} />
