...
Operational doc for distributing `me.chliny.opencode.mesh` via F-Droid (mainline)
...
3. Create `metadata/me.chliny.opencode.mesh.yml` from `distribution/fdroid-submission/metadata.yml`.
...
- App on IzzyOnDroid: add the accepted `me.chliny.opencode.mesh` package URL after acceptance.
