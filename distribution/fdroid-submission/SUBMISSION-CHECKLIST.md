...
https://gitlab.com/fdroid/fdroiddata to add `me.chliny.opencode.mesh` to the
...
git fetch upstream
git checkout -b add-me.chliny.opencode.mesh upstream/master
```
...
cp /path/to/opencode-mobile/distribution/fdroid-submission/metadata.yml \
   metadata/me.chliny.opencode.mesh.yml
```

Verify:
- `metadata/me.chliny.opencode.mesh.yml` exists
...
fdroid build me.chliny.opencode.mesh:<versionCode>
```

If the build fails, fix `metadata/me.chliny.opencode.mesh.yml` before filing the MR.
...
```bash
git add metadata/me.chliny.opencode.mesh.yml
git commit -m "Add me.chliny.opencode.mesh (OpenCode Mobile)"
git push origin add-me.chliny.opencode.mesh
```

Go to https://gitlab.com/<YOUR_GITLAB_USERNAME>/fdroiddata → open an MR
against `fdroid/fdroiddata:master`.

MR title: `Add me.chliny.opencode.mesh`
...
- Add `me.chliny.opencode.mesh` to F-Droid's inclusion notice in `docs/fdroid.md`.
