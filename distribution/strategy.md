...
|---|---|---|---|---|---|
| 1 | **Google Play (Internal)** | `me.chliny.opencode.zerotier` | ⏸ blocked on identity verification | Days after ID approved | CI ready. Track: `internal` first, then closed testing (12+ testers / 14d) before production. |
| 2 | **IzzyOnDroid** | `me.chliny.opencode.zerotier` (same key) | ❌ not started | 1–3 days | Submit prebuilt APK to https://codeberg.org/IzzyOnDroid/repodata/issues. Fastest OSS channel. |
| 3 | **F-Droid mainline** | `me.chliny.opencode.zerotier` (same key, reproducible build) | ❌ not started | 4–12 weeks | File MR at https://gitlab.com/fdroid/fdroiddata. Requires `expo-notifications` FCM audit + Sentry opt-in gate. |
| 4 | **Apple App Store** | `me.chliny.opencode.zerotier` | ⏸ pending iOS prep agent | Weeks (Apple enrollment $99 + review) | iOS agent running — separate report. |

**All channels: same package id (`me.chliny.opencode.zerotier`), same signing key.** Lets users update across stores in-place.
...
3. (manual) Create app `me.chliny.opencode.zerotier`. Fill listing from `play-listing.md`. Upload icon + feature graphic + screenshots. Complete Data safety + Content rating + App access.
...
2. Create `metadata/me.chliny.opencode.zerotier.yml` with reproducible-build config (template in F-Droid report).
